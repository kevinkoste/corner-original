...
// logic
import { useProfileContext, setEditing, updateComponent, updateEducation, deleteEducation } from '../context/ProfileContext'
...
export const EditEducation: React.FC<EducationComponent> = ({ id, props }) => {
  ...
  const { profileState, profileDispatch } = useProfileContext()
	...
	if (!profileState.editing && profileState.profile.components.find(comp => comp.type === 'education')?.props.education.length === 0) {
		return (
		...
		return (
			<ComponentContainer column width={12}>
				...
				{ profileState.profile.components.find(comp => comp.type === 'education')?.props.education.map((edu: EducationType, idx: number) => 
					<EducationEditRow
						key={idx}
						education={edu}
					/>
...
export const EducationRow: React.FC<EducationRowProps> = ({ education, color }) => {
	...
	return (
		<Div row width={12} style={{ alignItems:'top', margin:'15px 0px' }}>
			...
			<EducationTextContainer column width={12} style={{ color: color||'black' }}>
				<H2>{school}</H2>
				<H2>{degree}</H2>
				<H2>{date}</H2>
...
const EducationEditRow: React.FC<EducationRowProps> = ({ education, color }) => {
	...
	const handleClickAway = () => {
		profileDispatch(updateEducation({
			id: id,
			domain: domain,
			degree: degreeInput,
			school: schoolInput,
	...
	const handleDeleteEducation = () => {
		profileDispatch(deleteEducation({
			id: id,
			domain: domain,
			degree: degreeInput,
			school: schoolInput,
