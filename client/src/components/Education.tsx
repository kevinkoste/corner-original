...
export const EditEducation: React.FC<EducationComponent> = ({ id, props }) => {
  ...
  if (
    ...
    profileState.profile.components.find((comp) => comp.type === 'education')
      ?.props.education.length === 0
  ) {
    return (
      <ComponentContainer column width={12}>
        <H1 style={{ color: 'lightgray' }}>Education</H1>

        <Div column width={12} style={{ position: 'relative' }}>
          {placeholder.map((exp, idx) => (
            <EducationRow color={'lightgray'} key={idx} education={exp} />
          ))}
          <AddButton onClick={onAddClick}>Add education</AddButton>
        </Div>
      </ComponentContainer>
    )
