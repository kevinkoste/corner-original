...
export const EditExperience: React.FC<ExperienceComponent> = ({
  ...
  if (
    ...
    profileState.profile.components.find((comp) => comp.type === 'experiences')
      ?.props.experiences.length === 0
  ) {
    return (
      <ComponentContainer column width={12}>
        <H1 style={{ color: 'lightgray' }}>Experience</H1>

        <Div column width={12} style={{ position: 'relative' }}>
          {placeholder.map((exp, idx) => (
            <ExperienceRow color={'lightgray'} key={idx} experience={exp} />
          ))}
          <AddButton onClick={onAddClick}>Add experiences</AddButton>
        </Div>
      </ComponentContainer>
    )
